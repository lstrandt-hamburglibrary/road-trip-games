// Astro Blitz - High-speed arcade space shooter combining the best of Galaga and Galaxian
(function() {
    'use strict';

    // Game constants - FASTER AND MORE AGGRESSIVE!
    const CANVAS_WIDTH = 360;
    const CANVAS_HEIGHT = 480;
    const PLAYER_WIDTH = 24;
    const PLAYER_HEIGHT = 24;
    const PLAYER_SPEED = 5.5; // Faster movement
    const BULLET_WIDTH = 3;
    const BULLET_HEIGHT = 12;
    const BULLET_SPEED = 11; // Faster bullets
    const ENEMY_WIDTH = 20;
    const ENEMY_HEIGHT = 20;
    const ENEMY_BULLET_SPEED = 5; // Faster enemy bullets
    const MAX_PLAYER_BULLETS = 2;

    // Enemy types with colors and point values
    const ENEMY_TYPES = {
        BOSS: { color: '#FF00FF', points: 150, hits: 2, captureBeam: true }, // Magenta boss
        BUTTERFLY: { color: '#FF8800', points: 80, hits: 1, captureBeam: false }, // Orange
        BEE: { color: '#00FFFF', points: 50, hits: 1, captureBeam: false } // Cyan
    };
    ...
    class Player {
        ...
        draw(ctx) {
            ...
            // Draw main spaceship (bright cyan for Astro Blitz!)
    ...
    // Initialize game
    function initGame() {
        const canvas = document.getElementById('astroBlitzCanvas');
        if (!canvas) {
    ...
    function startStage() {
        // Every 3rd stage is a challenge stage (shorter timer!)
        if (gameState.level % 3 === 0) {
            gameState.challengeStage = true;
            gameState.challengeTimer = 240; // 4 seconds - faster!
        } else {
            gameState.challengeStage = false;
        }
    ...
    function createEnemyFormation() {
        ...
        for (let col = 3; col < 7; col++) {
            ...
            gameState.enemiesEntering.push({
                enemy: enemy,
                delay: enemyIndex * 5, // Faster formation entry!
                entered: false
            });
            enemyIndex++;
        }
    ...
    function drawChallengeStage(ctx) {
        ...
        ctx.fillText('⚡ BLITZ STAGE ⚡', CANVAS_WIDTH / 2, 30);
        ctx.textAlign = 'left';
    }
    ...
    function setupMobileControls() {
        ...
        const btnLeft = document.getElementById('astroBlitzBtnLeft');
        const btnRight = document.getElementById('astroBlitzBtnRight');
        const btnFire = document.getElementById('astroBlitzBtnFire');
    ...
    // Launch Astro Blitz
    window.launchAstroBlitz = function() {
        document.getElementById('gamesMenu').style.display = 'none';
        document.getElementById('astroBlitzGame').style.display = 'block';
        initGame();
        setupMobileControls();
    };

    // Exit to menu
    window.exitAstroBlitz = function() {
        if (gameState.animationId) {
            cancelAnimationFrame(gameState.animationId);
        }
        document.getElementById('astroBlitzGame').style.display = 'none';
