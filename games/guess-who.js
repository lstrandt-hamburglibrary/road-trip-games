...
(function() {
    ...
    const guessWhoState = {
        ...
        opponentEliminated: [], // Character IDs opponent eliminated (pass-and-play only)
                ...
                <div style="background: white; padding: 2rem; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                    ...
                    <div style="margin-top: 2rem; padding: 1rem; background: #f8f9fa; border-radius: 8px;">
                        ...
                        <ul style="margin: 0.5rem 0; padding-left: 1.5rem;">
                            ...
                            <li>Take turns asking yes/no questions about features</li>
                            <li>Eliminate characters based on answers</li>
                            <li>First to correctly guess the opponent's character wins!</li>
                        </ul>
                    </div>
                </div>
            </div>
            ...
            <div style="padding: 1rem; max-width: 1000px; margin: 0 auto;">
                ...
                <div style="background: white; padding: 1.5rem; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); margin-bottom: 1rem;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap: 0.75rem;">
                        ${characters.map(char => {
                            const isEliminated = guessWhoState.myEliminated.includes(char.id);
                            return `
                    ...
                    <p style="text-align: center; color: #666; margin-top: 1rem; font-size: 0.9rem;">Click to eliminate characters</p>
                </div>
                ...
                ${guessWhoState.gameOver ? `
                    <div style="text-align: center; padding: 1.5rem; background: ${guessWhoState.winner === 'player' ? '#43e97b' : '#f5576c'}; color: white; border-radius: 8px; margin-top: 1rem; font-size: 1.3rem; font-weight: bold;">
    ...
    window.toggleEliminate = function(characterId) {
        ...
        const index = guessWhoState.myEliminated.indexOf(characterId);
        if (index > -1) {
            guessWhoState.myEliminated.splice(index, 1);
        } else {
            guessWhoState.myEliminated.push(characterId);
        }
        renderGame();
    };

    // Make a guess
    window.makeGuess = function(characterId) {
        if (guessWhoState.gameOver) return;
        ...
        if (guessWhoState.gameMode === 'vs-ai') {
            ...
            if (character.id === guessWhoState.opponentSecret.id) {
                ...
                alert(`❌ Wrong! It's not ${character.name}. You lose!`);
                guessWhoState.gameOver = true;
            ...
            if (character.id === secret.id) {
                ...
                alert(`❌ Wrong! It's not ${character.name}. The other player wins!`);
                guessWhoState.gameOver = true;
